...
public class SuggerimentDao implements ISuggerimentDao {
	...
	public boolean acceptarSuggeriment(int idEspai, int idPlanta, int idHabitacio, String nomIaio, int idIaio, Date dataEntrada, Date dataSortida) {
		try {
            Connection conn = Conexion.conectar();
            String query = "INSERT into historial(id_espai, id_planta, id_habitacio, nom_iaio, id_iaio, data_entrada, data_sortida) values ('" + idEspai + "','" + idPlanta + "','" + idHabitacio + "','" + nomIaio + "','" + idIaio + "','" + dataEntrada + "','" + dataSortida + "')";
            Statement st = conn.createStatement();
            st.execute(query);
            st.close();
            ...
            HistorialDao.ah.add(new Historial(idEspai, idPlanta, idHabitacio, nomIaio, idIaio, dataEntrada, dataSortida));
