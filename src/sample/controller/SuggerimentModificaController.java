...
public class SuggerimentModificaController implements Initializable {
	...
	ArrayList<Integer> llId = SuggerimentVeureController.getId();
	ArrayList<String> llD = SuggerimentVeureController.getDates();
	...
	@FXML
	private Label lIdE,lIdP,lIdH,lNomI,lIdI,lDe,lDs;
	...
	public void initialize(URL url, ResourceBundle resourceBundle) {
		lIdE.setText(String.valueOf(llId.get(0)));
		lIdP.setText(String.valueOf(llId.get(1)));
		lIdH.setText(String.valueOf(llId.get(2)));
		//lNomI.setText(String.valueOf(llId.get(3)));
		lIdI.setText(String.valueOf(llId.get(3)));
		lDe.setText(String.valueOf(llD.get(0)));
		lDs.setText(String.valueOf(llD.get(1)));
		...
		for(int j=0;j<ae.size();j++) {
			...
			if(suma==ae.get(j).getNumPlaces()) {
				...
				ed.actualitzaDisponibilitat(ae.get(j));
			}
		}
		...
		idI.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<Integer>() {
			@Override
			public void changed(ObservableValue<? extends Integer> observableValue, Integer number, Integer number2) {
				lIdI.setDisable(true);

				int id = number2;
				for (int i = 0; i < ai.size(); i++) {
					if (id == ai.get(i).getId()) {
						nI.setText(ai.get(i).getNom());
						lNomI.setText(ai.get(i).getNom());
		...
		idH.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<Integer>() {
			@Override
			public void changed(ObservableValue<? extends Integer> observableValue, Integer number, Integer number2) {
				lIdH.setDisable(true);

			}
		});

		dE.valueProperty().addListener((observable, oldDate, newDate)->{
			lDe.setDisable(true);
		});

		dS.valueProperty().addListener((observable, oldDate, newDate)->{
			lDs.setDisable(true);
		});
	...
	protected void optBotons(ActionEvent event) throws IOException {
		...
		if (bot.equals("Tornar")) {
			...
			stage.show();
				...
				SuggerimentDao sd = new SuggerimentDao();
			;
				sd.eliminarSuggeriment(Integer.parseInt(lIdE.getText()), Integer.parseInt(lIdI.getText()));
			...
			if (s.crearSuggeriment(idE.getValue().intValue(), idP.getValue().intValue(), idH.getValue().intValue(),
				...
				labelOk.setText("Suggeriment modificat correctament");
